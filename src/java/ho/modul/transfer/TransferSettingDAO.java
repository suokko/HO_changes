// %266975537:hoplugins.transfers.dao%
...
public final class TransferSettingDAO {
    ...
    private static final String TABLE_NAME = "TRANSFERS_SETTINGS"; //$NON-NLS-1$
    ...
    private static void checkTable() {
        try {
        	...
        	DBZugriff.instance().getAdapter().executeUpdate("CREATE TABLE " + TABLE_NAME
                                                          + " (NAME VARCHAR(25),VALUE BOOLEAN)");
        	DBZugriff.instance().getAdapter().executeUpdate("CREATE INDEX plstatus_id ON "
                                                          + TABLE_NAME + " (NAME)");
        }

        try {
            final ResultSet rs = DBZugriff.instance().getAdapter().executeQuery("select NAME from "
                                                                              + TABLE_NAME); 
            rs.next();
        } catch (Exception e) {
        	DBZugriff.instance().getAdapter().executeUpdate("ALTER TABLE " + TABLE_NAME
                                                          + " ADD COLUMN NAME varchar(20)");
        	DBZugriff.instance().getAdapter().executeUpdate("UPDATE " + TABLE_NAME + " SET NAME=KEY");
        	DBZugriff.instance().getAdapter().executeUpdate("DROP INDEX plstatus_id ON " + TABLE_NAME);
        	DBZugriff.instance().getAdapter().executeUpdate("ALTER TABLE " + TABLE_NAME
                                                          + " DROP COLUMN KEY");
        	DBZugriff.instance().getAdapter().executeUpdate("CREATE INDEX plstatus_id ON "
                                                          + TABLE_NAME + " (NAME)");
