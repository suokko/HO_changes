...
final class SpielerNotizenTable extends AbstractTable {
	...
	String getSpielerNotiz(int spielerId) {
		...
		try {
			if (rs != null) {
				if (rs.first()) {
					return ho.core.db.DBManager.deleteEscapeSequences(rs.getString("Notiz"));
				} else {
					return "";
				}
			}
	...
	void saveSpielerNotiz(int spielerId, String notiz) {
		...
		if (spielerId > 0) {
			//erst UPdate versuchen
			try {
				statement = "UPDATE " + getTableName() + " SET Notiz='" + ho.core.db.DBManager.insertEscapeSequences(notiz) + "' WHERE SpielerID = " + spielerId;
	...
	private void createEntry(int spielerId, String notiz, String teamSmilie, String manualSmilie, boolean spielberechtigt, byte flag) {
		...
		statement += ("" + spielerId + ",'"+ho.core.db.DBManager.insertEscapeSequences(notiz)+"' ," + "'" + teamSmilie + "'," + "'" + manualSmilie + "',"+spielberechtigt+","+ flag +")");
