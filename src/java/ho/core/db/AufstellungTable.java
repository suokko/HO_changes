...
final class AufstellungTable extends AbstractTable {
	...
	Lineup getAufstellung(int hrfID, String name) {
		...
		if (auf != null) {
			...
			auf.setSubstitionList(new java.util.ArrayList<ISubstitution>(DBManager.instance().getMatchSubstitutionsByHrf(hrfID, name)));
		}
	...
	void saveAufstellung(int hrfId, Lineup aufstellung, String name) {
		...
		if (aufstellung != null) {
			...
			// Save Substitutions
			DBManager.instance().storeMatchSubstitutionsByHrf(hrfId, aufstellung.getSubstitutionList(), name);
