...
public class DividerDAO {
    ...
    public static void setDividerPosition(String key, int position) {
        ...
        int count = DBZugriff.instance().getAdapter().executeUpdate(query);

        if (count == 0) {
        	DBZugriff.instance().getAdapter().executeUpdate("insert into TRAININGEXPERIENCE_DIVIDER (NAME, POSITIONE) values ('" //$NON-NLS-1$
    ...
    public static int getDividerPosition(String key) {
        ...
        ResultSet rs = DBZugriff.instance().getAdapter().executeQuery(query);
    ...
    private static void checkTable() {
        try {
            ResultSet rs =DBZugriff.instance().getAdapter().executeQuery("select * from TRAININGEXPERIENCE_DIVIDER"); //$NON-NLS-1$
            rs.next();
        } catch (Exception e) {
        	DBZugriff.instance().getAdapter().executeUpdate("CREATE TABLE TRAININGEXPERIENCE_DIVIDER(NAME varchar(20),POSITIONE integer)"); //$NON-NLS-1$
        ...
        try {
            ResultSet rs = DBZugriff.instance().getAdapter().executeQuery("select NAME from TRAININGEXPERIENCE_DIVIDER"); //$NON-NLS-1$
            rs.next();
        } catch (Exception e) {
        	DBZugriff.instance().getAdapter().executeUpdate("ALTER TABLE TRAININGEXPERIENCE_DIVIDER ADD COLUMN NAME varchar(20)");
        	DBZugriff.instance().getAdapter().executeUpdate("UPDATE TRAININGEXPERIENCE_DIVIDER SET NAME=KEY");
        	DBZugriff.instance().getAdapter().executeUpdate("ALTER TABLE TRAININGEXPERIENCE_DIVIDER DROP COLUMN KEY");
        }
    }
}
