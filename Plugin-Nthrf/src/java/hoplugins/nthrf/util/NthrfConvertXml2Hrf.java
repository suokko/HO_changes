...
import hoplugins.nthrf.parser.NtTeamDetailsParser;
...
public class NthrfConvertXml2Hrf {
	...
	public final String createHrf(long teamId, IDownloadHelper dh, IXMLParser xp) throws Exception {
		...
		helper = Nthrf.getMiniModel().getHelper();
		this.teamId = teamId;
		try {
	...
	protected final void createBasics(NtTeamDetailsParser details, NtPlayersParser players) throws Exception {
		...
		m_sHRFBuffer.append("appversion=" + Nthrf.getMiniModel().getHOVersion() + "\n");
	...
	protected final void createTeam(NtTeamDetailsParser details) throws Exception {
		...
		if (details.getMorale()>-1 && details.getSelfConfidence()>-1) {
			...
			try {
				m_sHRFBuffer.append("stamning=" + Nthrf.getMiniModel().getTeam().getNameForTeamSpirit(details.getMorale()) + "\n");
			} catch (Exception e) {
			...
			try {
				m_sHRFBuffer.append("sjalvfortroende="+ Nthrf.getMiniModel().getTeam().getNameForSelfConfidence(details.getSelfConfidence()) + "\n");
			} catch (Exception e) {
