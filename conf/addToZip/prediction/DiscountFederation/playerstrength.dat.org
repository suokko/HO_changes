...
# stamina = stamina - 0.5		(staminaDelta)
# form = form - 0.5			(formDelta)
# xp = xp - 0.5				(xpDelta)
# if (stamina < 0) then stamina = 0	(staminaMin)
# if (form < 0) then form = 0		(formMin)
# if (xp < 0) then xp = 0		(xpMin)
# if (stamina > 8.25) then stamina = 8.25	(staminaMax)
# if (form > 7) then form = 7		(formMax)
# stamina = stamina/7			(staminaMultiplier)
# form = form/7				(formMultiplier)
# stamina = stamina^0.5			(staminaPower)
# form = form^0.5			(formPower)
# xp = xp^0.5				(xpPower)
...
#
# Id est:
# stamina-Factor = sqrt ((stamina-0.5)/7)
# form-Factor = sqrt ((form-0.5)/7)
...
# SkillSubDeltas:
# Default is to add 0.5 subskill for skills 0 (non existant) to 7 (solid)
