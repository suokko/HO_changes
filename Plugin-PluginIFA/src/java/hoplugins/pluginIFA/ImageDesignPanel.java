...
import javax.swing.event.ChangeListener;
   ...
   implements ActionListener
 {
  ...
  private boolean homeAway = true;
   ...
   FLAG_WIDTH = Integer.parseInt(ConfigManager.getTextFromConfig("visitedWidth"));
   ...
   add(this.northPanel, this.away, this.constraints, 1, 1, 1, 1);
     ...
     this.headerYesNo = new JCheckBox("Show Header", new Boolean(ConfigManager.getTextFromConfig("visitedHeaderShow")).booleanValue());
    ...
    this.footerYesNo = new JCheckBox("Show Footer", new Boolean(ConfigManager.getTextFromConfig("visitedFooterShow")).booleanValue());
    ...
    add(this.northPanel, this.footerYesNo, this.constraints, 1, 2, 1, 1);
     ...
     this.roundly = new JCheckBox("Roundly", new Boolean(ConfigManager.getTextFromConfig("visitedRoundly")).booleanValue());
   ...
   this.roundly.addChangeListener(changeListener);
    add(this.northPanel, this.roundly, this.constraints, 0, 3, 1, 1);
     ...
     this.greyColored = new JCheckBox("Grey", new Boolean(ConfigManager.getTextFromConfig("visitedGrey")).booleanValue());
   ...
   add(this.northPanel, this.greyColored, this.constraints, 1, 3, 1, 1);
     ...
     this.percentSlider = new JSlider(0, 100, Integer.parseInt(ConfigManager.getTextFromConfig("visitedBrightness")));
    ...
    this.percentSlider.setMinorTickSpacing(5);
     ...
     this.sizeSpinner = new JSpinner(new SpinnerNumberModel(FLAG_WIDTH, MIN_FLAG_WIDTH, MAX_FLAG_WIDTH, 1));
    ...
    add(this.northPanel, sizePanel, this.constraints, 0, 5, 1, 1);
     ...
     this.textField = new JTextField(ConfigManager.getTextFromConfig("visitedHeader"));
    ...
    add(this.northPanel, this.textField, this.constraints, 1, 5, 1, 1);
     ...
     this.animGif = new JCheckBox("Animated GIF", new Boolean(ConfigManager.getTextFromConfig("gifAnimated")).booleanValue());
     add(this.northPanel, this.animGif, this.constraints, 0, 6, 1, 1);
 
    JPanel spinnerPanel = new JPanel(new FlowLayout(1, 0, 0));
     this.delaySpinner = new JSpinner(new SpinnerNumberModel(Double.parseDouble(ConfigManager.getTextFromConfig("gifDelay")), 0.0D, 60.0D, 0.1D));
   ...
   private void add(JPanel panel, Component c, GridBagConstraints constraints, int x, int y, int w, int h) {
     constraints.gridx = x;
     constraints.gridy = y;
   ...
   private void setEmblemPanel(boolean homeAway)
   {
     ...
     try
     {
      if (this.emblemPanels[1] == null)
       {
       FLAG_WIDTH = Integer.parseInt(homeAway ? ConfigManager.getTextFromConfig("visitedWidth") : ConfigManager.getTextFromConfig("hostedWidth"));
        FlagLabel.BRIGHTNESS = Integer.parseInt(homeAway ? ConfigManager.getTextFromConfig("visitedBrightness") : ConfigManager.getTextFromConfig("hostedBrightness"));
       FlagLabel.GREY = new Boolean(homeAway ? ConfigManager.getTextFromConfig("visitedGrey") : ConfigManager.getTextFromConfig("hostedGrey")).booleanValue();
        ...
        EmblemPanel emblemPanel = new EmblemPanel(this.pluginIfaPanel, flags, enabled);
        ...
        emblemPanel.setGrey(FlagLabel.GREY);
         ...
         String path = homeAway ? ConfigManager.getTextFromConfig("visitedPathEmblem") : ConfigManager.getTextFromConfig("hostedPathEmblem");
        if (!path.equals(""))
         {
        ...
        emblemPanel.setHeaderText(homeAway ? ConfigManager.getTextFromConfig("visitedHeader") : ConfigManager.getTextFromConfig("hostedHeader"));
        emblemPanel.setHeader(new Boolean(homeAway ? ConfigManager.getTextFromConfig("visitedHeaderShow") : ConfigManager.getTextFromConfig("hostedHeaderShow")).booleanValue());
        emblemPanel.setFooter(new Boolean(homeAway ? ConfigManager.getTextFromConfig("visitedFooterShow") : ConfigManager.getTextFromConfig("hostedFooterShow")).booleanValue());
   ...
   private class TextKeyListener extends KeyAdapter {
     ...
     public void keyReleased(KeyEvent arg0) {
      ImageDesignPanel.this.emblemPanels[1].setHeaderText(((JTextField)arg0.getSource()).getText());
     }
   }
   private class StateChangeListener implements ChangeListener {
     ...
     public void stateChanged(ChangeEvent arg0) {
       ...
       {
        ImageDesignPanel.this.emblemPanels[1].setBrightness(((JSlider)arg0.getSource()).getValue());
        ImageDesignPanel.this.refreshFlagPanel();
       }
     }
   }
