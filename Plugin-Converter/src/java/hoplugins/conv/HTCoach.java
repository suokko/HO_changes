...
public class HTCoach extends HrfMaker {
            ...
            for (int i = 0; i < xmls.length; i++) {
                if ((xmls[i][0] == null)
                    ...
                    handleException(null,
                                    RSC.PROP_FILE_NOT_FOUND + " :" + filter
    ...
    private File[][] getFiles(String[] filter, File[] selectedFiles) {
        ...
        for (int i = 0; i < selectedFiles.length; i++) {
            xmls[i][0] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[0]
                                  + File.separator + RSC.HTCOACH_EXTENSION[0] + "_" + filter[i]
                                  + ".xml");
            xmls[i][1] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[1]
                                  + File.separator + RSC.HTCOACH_EXTENSION[1] + "_" + filter[i]
                                  + ".xml");
            xmls[i][2] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[2]
                                  + File.separator + RSC.HTCOACH_EXTENSION[2] + "_" + filter[i]
                                  + ".xml");
            xmls[i][3] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[3]
                                  + File.separator + RSC.HTCOACH_EXTENSION[3] + "_" + filter[i]
                                  + ".xml");
            xmls[i][4] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[4]
                                  + File.separator + RSC.HTCOACH_EXTENSION[4] + "_" + filter[i]
                                  + ".xml");
            xmls[i][5] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[5]
                                  + File.separator + RSC.HTCOACH_EXTENSION[5] + "_" + filter[i]
                                  + ".xml");
            xmls[i][6] = new File(dir.getPath() + File.separator + RSC.HTCOACH_EXTENSION[6]
                                  + File.separator + RSC.HTCOACH_EXTENSION[6] + "_" + filter[i]
                                  + ".xml");
        }
