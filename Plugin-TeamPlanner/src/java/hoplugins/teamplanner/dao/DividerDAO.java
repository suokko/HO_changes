// %2070157521:hoplugins.teamplanner.dao%
...
public class DividerDAO {
    ...
    public static void setDividerPosition(String key, int position) {
        String query = "update TEAMPLANNER_DIVIDER set POSITION = " + position + " where KEY = '"
                       + key + "'";
        int count = Commons.getModel().getAdapter().executeUpdate(query);

        if (count == 0) {
            Commons.getModel().getAdapter().executeUpdate("insert into TEAMPLANNER_DIVIDER (KEY, POSITION) values ('"
                                                          + key + "', " + position + ")");
        }
    }
