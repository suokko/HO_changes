...
public class TSPanel extends JPanel {
  ...
  public static final ImageIcon m_newImage   = new ImageIcon("hoplugins" + File.separator + "tsforecast" + File.separator + "new.png");
  public static final ImageIcon m_startImage = new ImageIcon("hoplugins" + File.separator + "tsforecast" + File.separator + "start.png");
  ...
  public void addCurve( Curve curve, boolean first) { 
    if( first) m_Curves.add( 0, curve); 
    else m_Curves.add( curve); 
  }
  
  public void addCurve( Curve curve) { addCurve( curve, false); }
  public boolean removeCurve( Curve curve) { return m_Curves.remove( curve); }
  public void showTeamspiritScale( boolean b) { m_bShowTeamspiritScale = b; }
  public void showConfidenceScale( boolean b) { m_bShowConfidenceScale = b; }
  ...
  private void drawDiagram( Graphics2D graphics2d) {
    ...
    try {
      for(int i = 0; i < m_Curves.size(); i++) {
        if(Class.forName("hoplugins.tsforecast.TrainerCurve").isInstance(m_Curves.get(i))) {
          drawIndicators(graphics2d, m_Curves.get(i));
        } else if(Class.forName("hoplugins.tsforecast.HistoryCurve").isInstance(m_Curves.get(i))) {
// For future use
//          drawCurveArea(graphics2d, m_Curves.get(i));
      ...
      ErrorLog.writeln( classnotfoundexception.toString());
    }
  }
...
*/
  ...
  // Hidden Feature "Show Values"        
        int showValues = (new Integer(TSForecast.m_Configuration.getProperty("ShowValues", "0"))).intValue();
        if(showValues > 0) {
          if(  (showValues & 1) > 0 
            && ( curve.getMatchType() == Curve.LEAGUE_MATCH || curve.getMatchType() == Curve.RELEGATION_MATCH))
              graphics2d.drawString( String.format("%3.2f", new Object[] { Double.valueOf(curve.getSpirit())}),
                                     (int)(x * m_dFactor + (double)m_iCoordX0), m_iMaxY + 2*DYFrame - 2 - iSpirit);
          else if((showValues & 2) > 0 && curve.getMatchType() == Curve.CUP_MATCH)
              graphics2d.drawString( String.format("%3.2f", new Object[] { Double.valueOf(curve.getSpirit()) }),
                                     (int)(x * m_dFactor + (double)m_iCoordX0), m_iMaxY + 2*DYFrame - 2 - iSpirit);
          else if((showValues & 4) > 0 && curve.getMatchType() == Curve.MASTERS_MATCH)
              graphics2d.drawString( String.format("%3.2f", new Object[] { Double.valueOf(curve.getSpirit()) }),
                                     (int)(x * m_dFactor + (double)m_iCoordX0), m_iMaxY + 2*DYFrame - 2 - iSpirit);
          else if((showValues & 8) > 0 && curve.getMatchType() == Curve.UNKNOWN_MATCH)
              graphics2d.drawString( String.format("%3.2f", new Object[] { Double.valueOf(curve.getSpirit()) }),
                                     (int)(x * m_dFactor + (double)m_iCoordX0), m_iMaxY + 2*DYFrame - 2 - iSpirit);
  
          if((showValues & 0x80) > 0) //Debugfeature
              ErrorLog.writeln( "Debug value " + curve.getDate() + " " + curve.matchTypeToString(curve.getMatchType()) 
                              + " " + String.format("%3.2f", new Object[] { Double.valueOf(curve.getSpirit())} ));
        }
...
// For future use
  private void drawCurveArea( Graphics2D graphics2d, Curve curve) {
    ...
    if(curve.first()) {
      ...
      for(; flag; flag = curve.next()) {
        ...
        polygon1.addPoint((int)(x * m_dFactor + (double)m_iCoordX0), m_iMaxY + DYFrame - iSpirit - m_iMaxY/20);
        polygon2.addPoint((int)(x * m_dFactor + (double)m_iCoordX0), m_iMaxY + DYFrame - iSpirit + m_iMaxY/20);
      }

      for( int i = polygon2.npoints-1; i > 0 ; i--) {
        polygon1.addPoint( polygon2.xpoints[i], polygon2.ypoints[i]);
      }
      graphics2d.fillPolygon(polygon1);
    }
  }
  ...
  private void drawIndicators( Graphics2D graphics2d, Curve curve) {
    ...
    if(curve.first()) {
      ...
      if(j1 < 0) j1 += 7;
      IHelper ihelper = TSForecast.m_clModel.getHelper();
      for(; flag; flag = curve.next()) {
        gregoriancalendar.setTime(curve.getDate());
        ...
        switch(curve.getPointType()) {
          case Curve.TRAINER_DOWN_PT:
            ...
            graphics2d.drawString( TSForecast.m_Language.getProperty("trainer_down", "coach skilldown"), 
          ...
          case Curve.NEW_TRAINER_PT:
            ...
            graphics2d.drawString( TSForecast.m_Language.getProperty("trainer_exchange", "new coach"), 
                                   (int)((double)l * m_dFactor + (double)m_iCoordX0), (m_iMaxY + 10) - 2 - i1 - 2);
            graphics2d.drawString( ihelper.getNameForSkill((int)curve.getSpirit(), true), 
                                   (int)((double)l * m_dFactor + (double)m_iCoordX0), ((m_iMaxY + 10) - 2 - i1) + 24);
            break;
          case Curve.START_TRAINER_PT:
            ...
            graphics2d.drawString( TSForecast.m_Language.getProperty("trainer_start", "coach skill"), 
                                   (int)((double)l * m_dFactor + (double)m_iCoordX0), (m_iMaxY + 10) - 2 - i1 - 2);
            graphics2d.drawString( ihelper.getNameForSkill((int)curve.getSpirit(), true), 
                                   (int)((double)l * m_dFactor + (double)m_iCoordX0), ((m_iMaxY + 10) - 2 - i1) + 24);
            break;
...
//    if(m_dFactor < 1.0D) m_dFactor = 1.0D;
    ...
    graphics2d.setColor(Color.blue);
    graphics2d.drawString( TSForecast.m_clModel.getResource().getProperty( "Season", "Season") + " " + iSeason,
...
//ErrorLog.writeln("Saisonstart   : "+ iCurrentSeasonStart);
    ...
    if(iCurrentSeasonStart > 0) {
      ...
      if(iCurrentSeasonStart > iSeasonLength) {
        ...
        graphics2d.drawString( TSForecast.m_clModel.getResource().getProperty("Season", "Season") + " " + (iSeason - 1),
  ...
  private void drawWeeks( Graphics2D graphics2d) {
    ...
    int iSeasonWeek = Math.round( (float)TSForecast.m_clModel.getBasics().getSpieltag() - (float)m_iTodayPosition / 7.0f);
  ...
  private void setStartEndDate() {
    ...
    m_startDate.setTime( TSForecast.m_clModel.getBasics().getDatum());
  ...
  double getMaxTextWidth( Graphics2D graphics2d) {
    Font font = new Font( "SansSerif", 1, UserParameter.instance().schriftGroesse);
    IHelper ihelper = TSForecast.m_clModel.getHelper();
