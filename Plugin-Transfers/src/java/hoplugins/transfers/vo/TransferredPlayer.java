...
import hoplugins.transfers.dao.PlayerMatchesDAO;
...
public class TransferredPlayer {
     ...
     */
    public TransferredPlayer(ISpieler player) {
        age = player.getAlter();
        ...
        endWeek = Commons.getModel().getBasics().getSpieltag()
                  + (Commons.getModel().getBasics().getSeason() * 16);
    }
     ...
     */
    public TransferredPlayer(PlayerTransfer pt) {
        id = pt.getPlayerId();
     ...
     */
    public final void addTransfer(PlayerTransfer transfer) {
