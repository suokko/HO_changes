...
public final class DividerDAO {
    ...
    public static void setDividerPosition(String key, int position) {
        ...
        if (count == 0) {
            Commons.getModel().getAdapter().executeUpdate("insert into TRANSFERS_DIVIDER (NAME, POSITIONE) values ('" //$NON-NLS-1$
    ...
    private static void checkTable() {
        try {
            final ResultSet rs = Commons.getModel().getAdapter().executeQuery("select * from TRANSFERS_DIVIDER"); //$NON-NLS-1$
            rs.next();
        } catch (Exception e) {
            Commons.getModel().getAdapter().executeUpdate("CREATE TABLE TRANSFERS_DIVIDER(NAME varchar(20),POSITIONE integer)"); //$NON-NLS-1$
            setDividerPosition("HistoryTabDivider", 400); //$NON-NLS-1$
            setDividerPosition("TypeTabDivider", 400); //$NON-NLS-1$
        }

        try {
            final ResultSet rs = Commons.getModel().getAdapter().executeQuery("select NAME from TRANSFERS_DIVIDER"); //$NON-NLS-1$
            rs.next();
        } catch (Exception e) {
            Commons.getModel().getAdapter().executeUpdate("ALTER TABLE TRANSFERS_DIVIDER ADD COLUMN NAME varchar(20)");
            Commons.getModel().getAdapter().executeUpdate("UPDATE TRANSFERS_DIVIDER SET NAME=KEY");
            Commons.getModel().getAdapter().executeUpdate("ALTER TABLE TRANSFERS_DIVIDER DROP COLUMN KEY");
        }
    }
}
