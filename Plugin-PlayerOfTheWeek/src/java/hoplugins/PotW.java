...
//		debugWindow.setVisible(true);
	...
	private LinkedList getSpielerListe(IDebugWindow debugWindow) {
		IJDBCAdapter db = m_clModel.getAdapter();
		...
		java.sql.ResultSet rs;
			 ...
			 //getMatchIDs
			   ...
			   //getPlayersthatplayed
				LinkedList spieler = new LinkedList();
	...
	private MatchLineupPlayer[] calcBestPlayers(LinkedList spieler) {
		MatchLineupPlayer g,lwb,rwb,cd1,cd2,im1,im2,s1,s2,lw,rw = null;
				...
				ListIterator p = spieler.listIterator(0);
				while (p.hasNext()) {
	...
	private void fillLineup( plugins.LineupPanel lineupPanel, LinkedList spieler )
			{
