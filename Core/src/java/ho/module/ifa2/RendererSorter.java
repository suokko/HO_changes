...
public class RendererSorter extends MouseAdapter {
	...
	private ImageIcon sortAZ = new ImageIcon(
			RendererSorter.class.getResource("image/ArrowTop.gif"));
	private ImageIcon sortZA = new ImageIcon(
			RendererSorter.class.getResource("image/ArrowUnder.gif"));

	public RendererSorter(JTable table, SortedTableModel sortedTableModel,
			int sortStartColumn) {
		this.table = table;
		this.sortedTableModel = sortedTableModel;
		this.oldIconColumn = sortStartColumn;
		this.sortColumn = sortStartColumn;
		((RendererDecorator) table.getColumnModel().getColumn(sortStartColumn)
				.getHeaderRenderer()).setLabelIcon(this.sortAZ);
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		TableColumn column = this.table.getColumnModel().getColumn(
				this.sortColumn);
		if (this.oldIconColumn != this.sortColumn)
			((RendererDecorator) this.table.getColumnModel()
					.getColumn(this.oldIconColumn).getHeaderRenderer())
					.setLabelIcon(null);
		this.oldIconColumn = this.sortColumn;
		RendererDecorator rendererDecorator = (RendererDecorator) column
				.getHeaderRenderer();
