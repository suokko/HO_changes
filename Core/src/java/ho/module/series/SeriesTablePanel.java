		...
		try {
			if (this.model.getCurrentSeries() != null) {
				final Font monospacedFont = new Font("monospaced", Font.PLAIN,
						ho.core.model.UserParameter.instance().schriftGroesse + 1);
				...
				for (int i = 0; i < tabelleneintraege.size(); i++) {
					...
					if (eintrag.getPunkte() > -1) {
						...
						((ColorLabelEntry) tableValues[j][8]).setSpezialNumber(
						...
						((ColorLabelEntry) tableValues[j][15]).setSpezialNumber(
						...
						((ColorLabelEntry) tableValues[j][22]).setSpezialNumber(
