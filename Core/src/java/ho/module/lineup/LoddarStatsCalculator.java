...
public class LoddarStatsCalculator {
	...
	public float calculate() {
		...
		// AiM or AoW or CA?
		if (tacticType == plugins.IMatchDetails.TAKTIK_MIDDLE) {
			AiM = (float) Math.min(tacticLevelAimAow, 20.0d);
			KZG = ZG + (((0.2f * (AiM - 1.0f)) / 19.0f) + 0.2f);
			KK = 0.0f;
		} else if (tacticType == plugins.IMatchDetails.TAKTIK_WINGS) {
			AoW = (float) Math.min(tacticLevelAimAow, 20.0d);
			KZG = ZG - (((0.2f * (AoW - 1.0f)) / 19.0f) + 0.2f);
			KK = 0.0f;
		} else if (tacticType == plugins.IMatchDetails.TAKTIK_KONTER) {
