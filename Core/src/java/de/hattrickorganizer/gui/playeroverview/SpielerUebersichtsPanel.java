...
public class SpielerUebersichtsPanel extends ImagePanel implements MouseListener, KeyListener {
	...
	public final void newSelectionInform() {
		...
		if (row > -1) {
			final de.hattrickorganizer.gui.utils.TableSorter model = m_jtSpielerUebersichtTable.getSorter();
    ...
    public final void update() {
        refresh();
		HOMainFrame.instance().getAufstellungsPanel().getAufstellungsTabelle().refresh();
    }
	...
	private void initComponents() {
		...
		// initSpielerDetail() );
		horizontalRightSplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, false, initSpielerDetail(), initSpielerHistory());
		verticalSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, false, tabelle, horizontalRightSplitPane);
	...
	private Component initSpielerDetail() {
		...
		tabbedPane.addTab(de.hattrickorganizer.model.HOVerwaltung.instance().getLanguageString("SpielerDetails"), scrollPane);
		...
		scrollPane.getVerticalScrollBar().setBlockIncrement(100);
		scrollPane.getVerticalScrollBar().setUnitIncrement(20);
		tabbedPane.addTab(de.hattrickorganizer.model.HOVerwaltung.instance().getLanguageString("Skilltester"), scrollPane);
