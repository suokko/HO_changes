...
public class XMLMatchLineupParser {
	...
	protected final Substitution createSubstitution(Element ele, int num) throws Exception {
		...
		if (tmp != null) {
			orderType = Byte.parseByte(XMLManager.instance().getFirstChildNodeValue(tmp));
		}
		...
		return new Substitution(playerOrderID, playerIn, playerOut, orderType, matchMinuteCriteria, pos, behaviour, card, standing);
	}
