...
public class RecapTableRenderer extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		...
		if ((row == 0) && ((table.getRowCount() > 1) || (table.getValueAt(0, 1) == null))) {
			...
			setFont(nFont);
			setBackground(UIColors.RecapRowColor); 
			setIcon(null);
		} else {
			int matchType = 0;
			...
			try {
				...
				setBackground(UIColors.getColor4Matchtyp(matchType));
			} catch (Exception e) {
			...
			case 1:
				...
				setIcon(Commons.getModel().getHelper().getImageIcon4Spieltyp(matchType));
				setText(null);
